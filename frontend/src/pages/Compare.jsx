...
import { compareQuotations, updateQuotation } from '../services/api'
...
export default function Compare() {
  ...
  const handleFinalizeAward = async () => {
    if (!winner) return
    if (window.confirm(`Finalize award for ${winner.vendor?.vendorName}?`)) {
      try {
        await updateQuotation(winner._id, { status: 'approved' })
        alert(`✅ Award finalized for ${winner.vendor?.vendorName}!`)
        handleCompare()
      } catch (err) {
        console.error(err)
      }
  ...
  const handleExportPDF = () => {
    const content = `
VendorFlow — RFQ Comparison Report
====================================
Title: ${title}
...
SUMMARY
-------
Total Quotes: ${results.length}
Lowest Quote: PKR ${lowestAmount?.toLocaleString()}
Total Variance: PKR ${(highestAmount - lowestAmount)?.toLocaleString()}
Winner: ${winner?.vendor?.vendorName} (${winner?.vendor?.companyName})

VENDOR RESPONSES
----------------
${results.sort((a, b) => a.amount - b.amount).map((q, i) => `
${i + 1}. ${q.vendor?.vendorName} (${q.vendor?.companyName})
   Amount: PKR ${q.amount?.toLocaleString()}
   Status: ${q.status}
   Delivery: ${q.deliveryTime || 'Not specified'}
   Warranty: ${q.warranty || 'Not specified'}
   Advance: ${q.advancePayment || 'Not specified'}
   Installation: ${q.installation || 'Not specified'}
`).join('')}

RISK ASSESSMENT
---------------
${winner?.vendor?.vendorName} provides the most competitive quote at PKR ${lowestAmount?.toLocaleString()}.
${highestAmount !== lowestAmount ? `Saving of PKR ${(highestAmount - lowestAmount)?.toLocaleString()} compared to highest quote.` : 'All vendors submitted equal quotes.'}

Generated by VendorFlow on ${new Date().toLocaleString()}
    `
    const blob = new Blob([content], { type: 'text/plain' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `RFQ-Comparison-${title.replace(/ /g, '-')}-${new Date().toLocaleDateString()}.txt`
    a.click()
  }
              ...
              <p className="text-sm text-gray-600 leading-relaxed">
                ...
                {results.length > 1 && highestAmount !== lowestAmount && (
                  <> This represents a saving of{' '}
                    <span className="font-semibold text-green-600">PKR {(highestAmount - lowestAmount).toLocaleString()}</span>{' '}
